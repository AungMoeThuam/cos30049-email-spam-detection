...
def create_pipeline(
    ...
    Returns:
        sklearn.pipeline.Pipeline with TfidfVectorizer and MultinomialNB.
    """
    pipeline = Pipeline([
...
def train(pipeline, X_train, y_train):
    """Train Naive Bayes pipeline on training data.
    
    Args:
        pipeline: sklearn.pipeline.Pipeline with TfidfVectorizer and MultinomialNB.
...
def get_feature_importance(pipeline, n_terms=15):
    ...
    Returns:
        Dictionary mapping class_label to list of (word, log_prob) tuples.
    """
    tfidf = pipeline.named_steps['tfidf']
    nb = pipeline.named_steps['naive_bayes']
    ...
    class_features = {}
    for class_id, probs in enumerate(log_probs):
        # Get indices of top n_terms features (highest log probability)
        top_indices = probs.argsort()[-n_terms:][::-1]
        top_words = [(feature_names[i], probs[i]) for i in top_indices]
